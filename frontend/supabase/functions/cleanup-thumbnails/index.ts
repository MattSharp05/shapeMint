...
serve(async (req) => {
  ...
  try {
    ...
    const { data: models, error: modelsError } = await supabase
      ...
      .select('id, thumbnail_angles, thumbnail_selected, thumbnail_custom')
      .not('thumbnail_angles', 'is', null)
      .not('thumbnail_selected', 'is', null)
      .eq('thumbnail_status', 'completed')
    ...
    for (const model of models) {
      try {
        ...
        if (filesToDelete.length > 0) {
          ...
          const updatedAngles = { [selectedAngle.toString()]: angles[selectedAngle.toString()] }
