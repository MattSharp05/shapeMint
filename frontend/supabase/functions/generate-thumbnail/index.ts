...
async function generateThumbnailForAngle(modelId: string, angle: number | string, glbUrl?: string): Promise<string> {
  ...
  if (glbUrl) {
    try {
      ...
      const previewUrl = glbUrl.replace('.glb', '_preview.jpg')
        .replace('.glb', '_thumb.jpg')
        .replace('/models/', '/previews/')
  ...
  const colors = {
    '0': '#3b82f6',      // Blue - Front view
    '45': '#8b5cf6',     // Purple - Diagonal
    '90': '#ef4444',     // Red - Side view  
    '135': '#f59e0b',    // Orange - Back diagonal
