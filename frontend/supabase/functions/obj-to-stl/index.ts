...
// OBJ to STL converter with automatic 50mm scaling for Slant3D compatibility
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2';
function convertOBJToSTLWithScaling(objText) {
  console.log('=== CONVERTING OBJ TO STL (50mm max) ===');
  ...
  // Scale to 50mm max dimension (perfect for desktop items)
  const targetMaxDimension = 50; // mm
  const scaleFactor = targetMaxDimension / originalMaxDimension;
  console.log('Scale factor for 50mm max:', scaleFactor.toFixed(4));
  ...
  const header = 'Scaled to 50mm max for Slant3D printing';
  for(let i = 0; i < 80; i++){
  ...
  for (const face of validFaces){
    ...
    // Vertex 1
    view.setFloat32(offset, vertex1[0], true);
    offset += 4;
    view.setFloat32(offset, vertex1[1], true);
    offset += 4;
    view.setFloat32(offset, vertex1[2], true);
    offset += 4;
    // Vertex 2
    view.setFloat32(offset, vertex2[0], true);
    offset += 4;
    view.setFloat32(offset, vertex2[1], true);
    offset += 4;
    view.setFloat32(offset, vertex2[2], true);
    offset += 4;
    // Vertex 3
    view.setFloat32(offset, vertex3[0], true);
    offset += 4;
    view.setFloat32(offset, vertex3[1], true);
    offset += 4;
    view.setFloat32(offset, vertex3[2], true);
    offset += 4;
  ...
  console.log('✅ STL conversion complete with 50mm scaling');
...
Deno.serve(async (req)=>{
  console.log('=== OBJ TO STL CONVERTER WITH 50MM SCALING ===');
  const corsHeaders = {
  ...
  try {
    ...
    console.log('Converting OBJ to STL with 50mm scaling:', objUrl);
    console.log('Task ID:', taskId);
    // Download OBJ file
    console.log('=== DOWNLOADING OBJ FILE ===');
    console.log('OBJ URL:', objUrl);
    // Try with headers that might be needed for Meshy URLs
    ...
    console.log('OBJ download response status:', objResponse.status);
    console.log('OBJ download response headers:', Object.fromEntries(objResponse.headers.entries()));
    if (!objResponse.ok) {
      const errorText = await objResponse.text();
      console.error('OBJ download error response:', errorText);
      throw new Error(`Failed to download OBJ file: ${objResponse.status} ${objResponse.statusText} - ${errorText}`);
    }
    const objText = await objResponse.text();
    console.log(`Downloaded OBJ: ${objText.length} characters`);
    // Convert OBJ to STL with scaling
    console.log('=== CONVERTING WITH 50MM SCALING ===');
    const { stlData, scalingInfo } = convertOBJToSTLWithScaling(objText);
    console.log(`Generated STL: ${stlData.byteLength} bytes`);
    // Upload STL to Supabase Storage (3d-models bucket)
    console.log('=== UPLOADING STL TO 3D-MODELS BUCKET ===');
    const stlFileName = fileName || `models/${taskId}.stl`;
    const { data: uploadData, error: uploadError } = await supabase.storage.from('3d-models').upload(stlFileName, stlData, {
    ...
    console.log('✅ STL uploaded successfully to 3d-models bucket:', uploadData);
    // Get public URL
    const { data: publicUrlData } = supabase.storage.from('3d-models').getPublicUrl(stlFileName);
