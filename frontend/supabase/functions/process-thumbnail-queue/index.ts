...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: queue, error: queueError } = await supabase.from('thumbnail_processing_queue').select('*').eq('status', 'pending').order('priority', {
      ascending: false
    }).order('created_at', {
      ascending: true
    }).limit(10);
    ...
    for (const item of queue){
      try {
        // Mark as processing
        await supabase.from('thumbnail_processing_queue').update({
          status: 'processing',
          processing_started_at: new Date().toISOString()
        }).eq('id', item.id);
        // Get model data
        const { data: model, error: modelError } = await supabase.from('generated_models').select('*').eq('id', item.model_id).single();
        if (modelError || !model) {
