...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!glbUrl || !modelId) {
      return new Response(JSON.stringify({ error: 'Missing glbUrl or modelId' }), {
        status: 400,
    ...
    if (!glbResponse.ok) {
      return new Response(JSON.stringify({ error: 'Failed to download GLB from Meshy' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    return new Response(JSON.stringify({ success: true, stlUrl: publicUrl }), {
      status: 200,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error) {
