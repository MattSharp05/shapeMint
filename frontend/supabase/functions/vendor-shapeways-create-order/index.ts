...
async function getModelMaterialPrice(token: string, modelId: string, materialId: string): Promise<number> {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    const resp = await fetchWithTimeout(`${SHAPEWAYS_API}/models/${modelId}/v1`, { headers: { 'Authorization': `Bearer ${token}` } });
    const json = await resp.json();
    lastJson = json;
    if (!resp.ok || json.result !== 'success') {
      ...
      if (!isNaN(price) && (price > 0 || (isPrintable && price >= 0))) {
        if (lastSeenPrice !== null && Math.abs(lastSeenPrice - price) < 0.000001) {
          try { console.log(JSON.stringify({ evt: 'order_price_stabilized', modelId, materialId, price, attempt })); } catch {}
          return price;
        }
        lastSeenPrice = price;
      }
      try { console.log(JSON.stringify({ evt: 'order_price_poll', attempt, modelId, materialId, price, isPrintable })); } catch {}
    }
    if (attempt < maxAttempts) await new Promise(r => setTimeout(r, delayMs));
  }
  try { console.error(JSON.stringify({ evt: 'order_model_info_poll_timeout', modelId, materialId, lastJson })); } catch {}
...
Deno.serve(async (req: Request) => {
  ...
  const originalFileName = modelUrl.split('/').pop()?.split('?')[0] || 'model.glb';
    ...
    if (priorQuote) {
      ...
      if (drift) {
        return new Response(JSON.stringify({ error: 'price_changed', message: 'Price changed since quote. Please confirm new price.', current: { itemSubtotal, surcharge, totalPrice } }), { status: 409, headers: corsHeaders });
      }
    }
  ...
  try { await client.from('order_events').insert({ order_id: inserted.id, user_id: userId, evt_type: 'order_created', evt_data: { totalPrice, itemSubtotal, surcharge, shippingPrice } }); } catch {}
    ...
    if (!vendorResp.ok || vendorJson.result !== 'success') {
      ...
      return new Response(JSON.stringify({ error: 'vendor_order_failed', vendorStatus: vendorJson }), { status: 502, headers: corsHeaders });
    }
  ...
  try { await client.from('order_events').insert({ order_id: inserted.id, user_id: userId, evt_type: 'order_submitted', evt_data: { vendorOrderId } }); } catch {}
    ...
    try { console.error(JSON.stringify({ evt: 'create_order_error', message: err.message, name: err.name })); } catch {}
    return new Response(JSON.stringify({ error: 'order_creation_failed', message: err.message }), { status: 500, headers: corsHeaders });
  }
});
