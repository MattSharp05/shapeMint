...
Deno.serve(async (req)=>{
  ...
  if (!orderId) return new Response(JSON.stringify({
  ...
  try {
    ...
    if (ordErr || !ord) return new Response(JSON.stringify({
      error: 'order_not_found'
    }), {
      status: 404,
      headers: corsHeaders
    });
    if (ord.user_id !== userId) return new Response(JSON.stringify({
      error: 'forbidden'
    }), {
      status: 403,
      headers: corsHeaders
    });
    if (!ord.vendor_order_id) return new Response(JSON.stringify({
    ...
    const vendorResp = await fetchWithTimeout(`${SHAPEWAYS_API}/orders/${ord.vendor_order_id}/v1`, {
      headers: {
    ...
    const vendorStatus = vendorJson.status || vendorJson.orderStatus || '';
