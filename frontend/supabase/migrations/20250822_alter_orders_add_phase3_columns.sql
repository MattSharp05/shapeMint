-- Backfill Phase 3 required columns on existing orders table (idempotent)
-- Fixes runtime error: missing material_id column during order insertion

begin;

-- Core additive columns (nullable for safe backfill; app enforces presence)
...
alter table orders add column if not exists material_id text; -- was NOT NULL in new schema; leave nullable for legacy rows
...
-- Sequence + order number generator (idempotent)
...
-- Conditionally create trigger if absent
DO $$
BEGIN
