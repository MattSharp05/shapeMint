...
export function Generate() {
  ...
  // Check for prefilled prompt from Explore page
  const prefilledPrompt = location.state?.prefilledPrompt;
  const socialTag = location.state?.socialTag;
  const prefilledMode = location.state?.mode;
  const prefilledImage = location.state?.image;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <GenerationForm 
              ...
              loading={generating}
              initialData={isEditing ? generationData : (prefilledPrompt ? { 
                prompt: prefilledPrompt, 
                mode: prefilledMode || 'text',
